...
- Review it whenever you change: **payload** — `services/llm_prompts.py` (the canonical source for what each prompt carries), `services/llm_service.py`'s provider/model list, or any `/ai/*` endpoint; or **trigger** — `hooks/useTagSuggestions.ts`, `hooks/useRelationshipSuggestions.ts`, `hooks/useArgumentSuggestions.ts`, `hooks/useAITagIntegration.ts`, or the components wiring them (e.g. `components/ContentCard/actions/AddTagAction.tsx`).
