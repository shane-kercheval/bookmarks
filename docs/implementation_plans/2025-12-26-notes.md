...
- No syntax highlighting initially (can add CodeMirror later if users request it)

**Rationale:**
- Many successful note apps use plain textareas (Bear, Notion markdown blocks, Obsidian source mode)
- Users writing markdown are comfortable with plain text
- Zero additional dependencies for editing
- Can upgrade to CodeMirror later if syntax highlighting while editing is requested
...
2. **Markdown editor:** Simple textarea for editing + react-markdown/remark-gfm/rehype-sanitize for viewing. Start simple, upgrade to CodeMirror later if needed.
