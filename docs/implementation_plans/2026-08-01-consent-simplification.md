...
2. **Backfill run against production — also before the merge, and this is a hard gate.** M3's migration drops `user_consents`, and Railway's `api` service runs `alembic upgrade head` before it starts. So the merge destroys the backfill's source data. Run it first and the timestamps are already in Clerk; run it after and they are gone permanently.
   ...
   The backfill does not need to be deployed to run: it is a standalone script an operator executes with `CLERK_SECRET_KEY` and an explicit `--database-url` pointing at production. It reads Postgres and writes Clerk, and depends on neither the gate's removal nor the frontend. Run it from the branch. Precondition, checked immediately before the run: `GET /consent/versions` on the production API still returns `2026-07-31` for both documents — this validates the enforced-pair pin the script's cohort report compares against (added 2026-08-02).
...
**Correction (2026-08-02, found in review).** Two refinements to the split as specified above. First, "current" must mean the pair **production enforces at run time** — `2026-07-31`/`2026-07-31`, pinned in the script — not the branch's constants, which M0 bumps in this same PR; compared against those, every row reads stale by construction and the split says nothing. The pin is verified operationally: immediately before the production run, `GET /consent/versions` on the target API must still return the pinned pair (it serves the deployed constants — exactly what the gate enforces). Second, the version strings and `consented_at` are **independent measurements** — the deleted `POST /consent/me` stored client-supplied version strings with length-only validation, while the timestamp was server-set — so the report presents the distinct version pairs recorded *and* the timestamp-based stop-condition count separately, computed from database rows alone (independent of Clerk reconciliation), and flags any row where the two disagree. The stop condition below is evaluated on the timestamp, the trustworthy column.
...
**Correction (2026-08-02, post-implementation review).** The implementation deviated: f8e5c71 deleted the wrapper anyway, folding its docstring — every piece of evidence the paragraph above names, verbatim — into `resolve_client_ip`, and rewrote `test_request_utils.py` accordingly. The cross-reference was inverted rather than orphaned, so the outcome this paragraph was protecting (the knowledge survives, attached to a live function) holds; what was violated was the instruction, not its purpose. Recorded rather than reverted. The deviation went unrecorded at the time, which is the actual defect.
...
- That evidence is thin. After M0 no surviving row names a current document — rows name the superseded July 2026 pair or older ones (users blocked at the gate never re-accepted; and the version strings were client-supplied to the accept endpoint, so arbitrary values are possible) — and dated policy-document history already records what changed and when. *The trade* calls the IP/user-agent capture "belt-and-suspenders on top of a required checkbox and a server-recorded timestamp."
...
A third option surfaced in review (2026-08-02): a one-time pre-drop export of the rows into the same restricted location the backfill's differing-rows report goes to, preserving the evidence with no live-database retention. The owner considered and declined it: roughly a dozen beta users, evidence that names only superseded documents, and fields nothing captures going forward — its value only decays. The drop proceeds without an export.
...
- A user whose consent row names **stale** versions likewise succeeds in each family. **[Corrected 2026-08-02:** this bullet predates the table-drop decision. Once `user_consents` is dropped in this same milestone, a stale row cannot be constructed — the stale branch of the deleted `_check_consent` is unreachable, not untested, and every user is permanently in the no-record state the bullet above covers. The stale-version tests existed at 69c1f17 and were deleted with the table in 00e2db4; the rationale lives in `tests/test_consent_gate_removed.py`'s module docstring.**]**
...
**Scope addition (2026-08-02, found in review):** `README.md`'s "Updating Privacy Policy or Terms of Service" section was a second copy of this runbook, still promising a re-consent dialog and citing the deleted `/consent/status` endpoint — outside every milestone's grep scope because it sits above `docs/`. Rewritten as part of this milestone to describe the notice model and point at the runbook here, so the obligation has one authoritative home.
...
`docs/architecture.md` carries the most consent detail and needs the most care. Known references: the store list (~105), the interceptor's status handling (~110), the auth-layer description (~183), the `UserConsent` row in the models table (~211 — added to this list 2026-08-02; it was missing), all of §5 "Authentication, consent, and request identity" (~250–289, including the dependency table listing the two removed variants), the "consent router established commit-then-invalidate" attribution (~289), the Redis key-schema table (~385, which names `auth:v7:` and must move to v8), and the error-code table (~442).

**Additions (2026-08-02, found in review):** `docs/ios-clerk-migration-guide.md:43,50` still describes the 451 gate as live guidance for the iOS client — inside the DoD's grep scope but absent from the list above, so it is named here explicitly. And the close-out grep must extend beyond `docs/` and `frontend/public/` to the repo root (`README*`): the README's stale consent runbook sat exactly there (fixed under M6's scope addition). Re-read §5 as a whole rather than patching lines — its narrative of the request lifecycle changes shape, and the `user_consents` table is gone entirely, so remove its references rather than rewriting them — the "Things that are easy to miss" section should record that Clerk's `legalAcceptedAt` is now the only per-user acceptance record, and that it carries a timestamp but not which versions were accepted.
