...
- **J6 — EXECUTED 2026-07-31 (irreversible).** Preflight passed (zero account deletions since 2026-07-31, so no Auth0 identities needed reconciliation); operator deleted both Auth0 tenants (production and development) in the Auth0 dashboard. **Auth0 no longer exists in any Tiddly surface.** Post-J6 record cleanup landed in the closing commit: README_DEPLOY's Auth0-side reconciliation subsection deleted, and the temporary repo-wide Auth0-reference gate retired (see the note below).

**On the reference gate (retired 2026-07-31).** A `test_no_auth0_references.py` tripwire was built during the contract phase to make the J4 grep gate mechanical. It earned its keep once: the sweep it forced found four real stragglers (dead `Settings(auth0_*)` kwargs that pydantic was silently ignoring, two stale test names, seeded demo content misdescribing the app's own auth, and an enum table documenting `AUTH0` as a current value). With the sweep done and the tenants deleted, keeping it would have meant maintaining a 22-entry path allowlist forever to guard against a reintroduction that can no longer happen — so it was deleted in the closing commit. The two guards that genuinely protect against reintroduction are permanent and live elsewhere: the eslint ban on importing `@auth0/auth0-react` (the only mechanical re-entry path) and `test_auth_clerk.py`'s old-issuer rejection test (a behavioral test worth keeping on its own merits).
