...
from services.bookmark_service import archive_bookmark, create_bookmark, delete_bookmark
from services.content_service import search_all_content
from services.note_service import archive_note, create_note, delete_note
...
async def test__search_all_content__returns_both_bookmarks_and_notes(
    ...
    await create_bookmark(db_session, test_user.id, bookmark_data)

    # Create a note
    note_data = NoteCreate(title='Example Note')
    await create_note(db_session, test_user.id, note_data)
...
async def test__search_all_content__has_correct_type_discriminator(
    ...
    bookmark = await create_bookmark(db_session, test_user.id, bookmark_data)

    note_data = NoteCreate(title='Note')
    note = await create_note(db_session, test_user.id, note_data)
...
async def test__search_all_content__view_active_excludes_deleted(
    ...
    active_bookmark = await create_bookmark(db_session, test_user.id, bookmark_data)

    note_data = NoteCreate(title='Deleted Note')
    deleted_note = await create_note(db_session, test_user.id, note_data)
    await db_session.flush()

    # Delete the note
    await delete_note(db_session, test_user.id, deleted_note.id)
    await db_session.flush()
...
async def test__search_all_content__view_active_excludes_archived(
    ...
    archived_bookmark = await create_bookmark(db_session, test_user.id, bookmark_data)

    note_data = NoteCreate(title='Active Note')
    active_note = await create_note(db_session, test_user.id, note_data)
    await db_session.flush()

    # Archive the bookmark
    await archive_bookmark(db_session, test_user.id, archived_bookmark.id)
    await db_session.flush()
...
async def test__search_all_content__view_archived_returns_only_archived(
    ...
    archived_note = await create_note(db_session, test_user.id, note_data)

    note_data2 = NoteCreate(title='Active Note')
    await create_note(db_session, test_user.id, note_data2)
    await db_session.flush()

    # Archive bookmark and note
    await archive_bookmark(db_session, test_user.id, archived_bookmark.id)
    await archive_note(db_session, test_user.id, archived_note.id)
    await db_session.flush()
...
async def test__search_all_content__view_deleted_returns_all_deleted(
    ...
    deleted_bookmark = await create_bookmark(db_session, test_user.id, bookmark_data)
    ...
    await delete_bookmark(db_session, test_user.id, deleted_bookmark.id)
    await delete_note(db_session, test_user.id, deleted_note.id)
    await db_session.flush()
...
async def test__search_all_content__sort_by_created_at_desc(
    ...
    await create_bookmark(db_session, test_user.id, bookmark_data2)
    await db_session.flush()
...
async def test__search_all_content__excludes_other_users_content(
    ...
    await create_bookmark(db_session, other_user.id, other_bookmark_data)

    other_note_data = NoteCreate(title='Other Note')
    await create_note(db_session, other_user.id, other_note_data)
